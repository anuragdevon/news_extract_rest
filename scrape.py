    ...
    if url:
        ...
        if error == "":
            ...
            try:
                ...
                cur.execute(
                    "SELECT url FROM articles WHERE url = %s", (raw_data["link"],)
                )
                article = cur.fetchone()
                if article is None:
                    cur.execute(
                        "INSERT INTO articles (url, title, author, pub_date, timestamp) VALUES (%s, %s, %s, %s, %s)",
                        (
                            raw_data["link"],
                            raw_data["title"],
                            raw_data["author"],
                    ...
                    cur.execute(
                        "UPDATE articles SET title = %s, author = %s, pub_date = %s, timestamp = %s WHERE url = %s",
                        (
                            raw_data["title"],
                            raw_data["author"],
                            raw_data["pub_date"],
